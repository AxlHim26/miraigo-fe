import AutoAwesomeIcon from "@mui/icons-material/AutoAwesome";
import ForumIcon from "@mui/icons-material/Forum";
import GraphicEqIcon from "@mui/icons-material/GraphicEq";
import GridViewIcon from "@mui/icons-material/GridView";
import HistoryRoundedIcon from "@mui/icons-material/HistoryRounded";
import LayersOutlinedIcon from "@mui/icons-material/LayersOutlined";
import LibraryBooksIcon from "@mui/icons-material/LibraryBooks";
import MapIcon from "@mui/icons-material/Map";
import MenuBookIcon from "@mui/icons-material/MenuBook";
import MicIcon from "@mui/icons-material/Mic";
import PsychologyIcon from "@mui/icons-material/Psychology";
import RecordVoiceOverIcon from "@mui/icons-material/RecordVoiceOver";
import SchoolIcon from "@mui/icons-material/School";
import SearchIcon from "@mui/icons-material/Search";
import StyleIcon from "@mui/icons-material/Style";
import SubtitlesIcon from "@mui/icons-material/Subtitles";

import { grammarLevels } from "@/core/data/grammar";
import type { NavSection, TopTab } from "@/shared/types/navigation";

export const topTabs: TopTab[] = [
  {
    id: "vocabulary",
    label: "Từ Vựng",
    href: "/vocabulary",
    icon: <LibraryBooksIcon fontSize="small" />,
  },
  {
    id: "grammar",
    label: "Ngữ Pháp",
    href: "/grammar",
    icon: <MenuBookIcon fontSize="small" />,
  },
  {
    id: "kanji-map",
    label: "Hán Tự",
    href: "/kanji-map",
    icon: <MapIcon fontSize="small" />,
  },
  {
    id: "courses",
    label: "Khoá Học",
    href: "/courses",
    icon: <SchoolIcon fontSize="small" />,
  },
  {
    id: "practice",
    label: "Luyện Tập",
    href: "/practice/chat",
    icon: <AutoAwesomeIcon fontSize="small" />,
  },
];

export const grammarSidebarSections: NavSection[] = [
  {
    id: "overview",
    items: [
      {
        id: "home",
        label: "Trang chủ",
        href: "/grammar",
        icon: <GridViewIcon fontSize="small" />,
      },
      {
        id: "search",
        label: "Tìm kiếm",
        icon: <SearchIcon fontSize="small" />,
        badge: "(Ctrl + K)",
        variant: "plain",
      },
    ],
  },
  {
    id: "courses",
    title: "Giáo trình",
    items: grammarLevels.map((level) => ({
      id: level.id,
      label: level.title,
      href: `/grammar/${level.id}`,
      variant: "plain",
    })),
  },
];

export const vocabularySidebarSections: NavSection[] = [
  {
    id: "overview",
    items: [
      {
        id: "home",
        label: "Trang chủ",
        href: "/vocabulary",
        icon: <GridViewIcon fontSize="small" />,
      },
      {
        id: "search",
        label: "Tìm kiếm",
        icon: <SearchIcon fontSize="small" />,
        badge: "(Ctrl + K)",
        variant: "plain",
      },
      {
        id: "review",
        label: "Cần ôn tập",
        icon: <PsychologyIcon fontSize="small" />,
        variant: "plain",
      },
    ],
  },
  {
    id: "recent",
    title: "GẦN ĐÂY",
    meta: "0/5",
    items: [
      {
        id: "create-set",
        label: "Tạo bộ từ mới",
        icon: <LayersOutlinedIcon fontSize="small" />,
        variant: "dashed",
      },
    ],
  },
  {
    id: "vocabulary-library",
    title: "Kho từ vựng",
    items: [
      {
        id: "vocabulary-n5",
        label: "Từ vựng N5",
        href: "/vocabulary/vocab-n5-core",
        variant: "plain",
      },
      {
        id: "vocabulary-n4",
        label: "Từ vựng N4",
        href: "/vocabulary/vocab-n4-core",
        variant: "plain",
      },
      {
        id: "vocabulary-n3",
        label: "Từ vựng N3",
        href: "/vocabulary/vocab-n3-core",
        variant: "plain",
      },
      {
        id: "vocabulary-n2",
        label: "Từ vựng N2",
        href: "/vocabulary/vocab-n2-core",
        variant: "plain",
      },
      {
        id: "vocabulary-n1",
        label: "Từ vựng N1",
        href: "/vocabulary/vocab-n1-core",
        variant: "plain",
      },
      {
        id: "vocabulary-poilerpale",
        label: "Poilerpale",
        href: "/vocabulary/vocab-poilerpale",
        variant: "plain",
      },
      {
        id: "vocabulary-daily",
        label: "Giao tiếp hằng ngày",
        href: "/vocabulary/vocab-daily-conversation",
        variant: "plain",
      },
      {
        id: "vocabulary-travel",
        label: "Du lịch & di chuyển",
        href: "/vocabulary/vocab-travel-transport",
        variant: "plain",
      },
      {
        id: "vocabulary-work",
        label: "Công việc & văn phòng",
        href: "/vocabulary/vocab-work-office",
        variant: "plain",
      },
      {
        id: "vocabulary-food",
        label: "Nhà hàng & đồ ăn",
        href: "/vocabulary/vocab-food-restaurant",
        variant: "plain",
      },
      {
        id: "vocabulary-health",
        label: "Y tế & sức khỏe",
        href: "/vocabulary/vocab-health-medical",
        variant: "plain",
      },
      {
        id: "vocabulary-tech",
        label: "Công nghệ & IT",
        href: "/vocabulary/vocab-technology-it",
        variant: "plain",
      },
      {
        id: "vocabulary-business",
        label: "Kinh doanh",
        href: "/vocabulary/vocab-business",
        variant: "plain",
      },
      {
        id: "vocabulary-school",
        label: "Trường học",
        href: "/vocabulary/vocab-school-campus",
        variant: "plain",
      },
      {
        id: "vocabulary-emotion",
        label: "Cảm xúc & tính cách",
        href: "/vocabulary/vocab-emotion-personality",
        variant: "plain",
      },
      {
        id: "vocabulary-specialized",
        label: "Từ vựng chuyên ngành",
        href: "/vocabulary/vocab-specialized",
        variant: "plain",
      },
    ],
  },
];

export const practiceSidebarSections: NavSection[] = [
  {
    id: "practice",
    title: "Khu vực luyện tập",
    items: [
      {
        id: "chat",
        label: "Trò chuyện AI",
        href: "/practice/chat",
        icon: <AutoAwesomeIcon fontSize="small" />,
      },
      {
        id: "practice-reading",
        label: "Luyện đọc",
        href: "/practice/reading",
        icon: <MenuBookIcon fontSize="small" />,
      },
      {
        id: "practice-listening",
        label: "Luyện nghe",
        href: "/practice/listening",
        icon: <GraphicEqIcon fontSize="small" />,
      },
      {
        id: "practice-shadowing",
        label: "Shadowing",
        href: "/practice/shadowing",
        icon: <RecordVoiceOverIcon fontSize="small" />,
      },
      {
        id: "flashcard",
        label: "Flashcard",
        href: "/practice/flashcard",
        icon: <StyleIcon fontSize="small" />,
      },
      {
        id: "jlpt",
        label: "Luyện thi JLPT",
        href: "/practice/jlpt",
        icon: <AutoAwesomeIcon fontSize="small" />,
      },
    ],
  },
  {
    id: "ai-voice",
    title: "AI Voice",
    items: [
      {
        id: "pronunciation",
        label: "Luyện phát âm",
        href: "/practice/pronunciation",
        icon: <MicIcon fontSize="small" />,
      },
      {
        id: "transcribe",
        label: "Gỡ băng AI",
        href: "/practice/transcribe",
        icon: <SubtitlesIcon fontSize="small" />,
      },
      {
        id: "conversation",
        label: "Hội thoại Realtime",
        href: "/practice/conversation",
        icon: <ForumIcon fontSize="small" />,
      },
      {
        id: "ai-shadowing",
        label: "Shadowing AI",
        href: "/practice/shadowing",
        icon: <RecordVoiceOverIcon fontSize="small" />,
      },
    ],
  },
];

export const coursesSidebarSections: NavSection[] = [
  {
    id: "overview",
    items: [
      {
        id: "home",
        label: "Trang chủ",
        href: "/courses",
        icon: <GridViewIcon fontSize="small" />,
      },
      {
        id: "search",
        label: "Tìm kiếm",
        icon: <SearchIcon fontSize="small" />,
        badge: "Ctrl + K",
      },
      {
        id: "continue-learning",
        label: "Tiếp tục xem",
        icon: <HistoryRoundedIcon fontSize="small" />,
        variant: "plain",
      },
    ],
  },
];

export const kanjiSidebarSections: NavSection[] = [
  {
    id: "overview",
    items: [
      {
        id: "home",
        label: "Lộ trình",
        href: "/kanji-map",
        icon: <GridViewIcon fontSize="small" />,
      },
      {
        id: "kanji-lookup",
        label: "Tra cứu",
        href: "/kanji-map/lookup",
        icon: <SearchIcon fontSize="small" />,
      },
      {
        id: "kanji-review",
        label: "Cần ôn tập",
        href: "/kanji-map/review",
        icon: <HistoryRoundedIcon fontSize="small" />,
      },
    ],
  },
  {
    id: "kanji-library",
    title: "Kho hán tự",
    items: [
      {
        id: "kanji-n5",
        label: "Hán tự N5",
        href: "/kanji-map/n5",
        variant: "plain",
      },
      {
        id: "kanji-n4",
        label: "Hán tự N4",
        href: "/kanji-map/n4",
        variant: "plain",
      },
      {
        id: "kanji-n3",
        label: "Hán tự N3",
        href: "/kanji-map/n3",
        variant: "plain",
      },
      {
        id: "kanji-n2",
        label: "Hán tự N2",
        href: "/kanji-map/n2",
        variant: "plain",
      },
      {
        id: "kanji-n1",
        label: "Hán tự N1",
        href: "/kanji-map/n1",
        variant: "plain",
      },
    ],
  },
];
