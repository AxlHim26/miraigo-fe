...
const CACHE_FILE =
  "/run/media/jun/999d1d27-a5c5-4898-bdf4-179df4856a87/data/kanji-translation-cache.json";
...
async function processFile(filePath) {
  ...
  // ROOT KANJI
  const rootMeaningEn =
    data.kanjialiveData?.kanji?.meaning?.english ||
    (data.jishoData?.meaning ? data.jishoData.meaning : null);
...
async function main() {
  ...
  const files = fs
    ...
    .filter((f) => f.endsWith(".json") && !f.startsWith("CDP"));
