 ...
 * - Đọc data/kanji-processed.txt
 * - Chọn 15 file .json trong data/kanji/ chưa xử lý (ưu tiên tên ngắn)
 * - Dịch meaning EN → VI (string thay bằng VI, object thêm vietnamese)
 * - Ghi file, append 15 tên vào kanji-processed.txt
 *
 * Chạy: node scripts/process-15-kanji-vi.mjs
 * Cần MEGALLM_API_KEY trong .env.local
 */
...
async function translateBatch(texts, attempt = 1) {
  ...
  if (!response.ok) {
    ...
    if (isRateLimit && attempt <= 5) {
      ...
      console.warn(`⏳ Rate limit. Đợi ${waitSec}s (lần ${attempt}/5)...`);
...
if (listOnly) {
  console.log("Đã xử lý (--list-only): " + toProcess.join(", "));
  process.exit(0);
}
