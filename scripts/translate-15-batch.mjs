 ...
 * - Đọc kanji-processed.txt
 * - Chọn 15 file .json chưa xử lý (ưu tiên tên ngắn)
 * - Dịch meaning (string -> VI, object.english -> thêm vietnamese)
 * - Ghi file, append 15 tên vào kanji-processed.txt
 * Chạy: node scripts/translate-15-batch.mjs
 */
...
// 3) Sắp xếp: ưu tiên tên ngắn (số ký tự tên file không .json)
...
const chunkSize = 4;
const delayMs = Math.max(0, Number(process.env.MEGALLM_BATCH_DELAY_MS) || 2000);
...
// Append 15 tên vào kanji-processed.txt
const lineEnd = processedRaw.length > 0 && !processedRaw.endsWith("\n") ? "\n" : "";
await fs.appendFile(processedPath, lineEnd + toProcess.join("\n") + "\n", "utf-8");
