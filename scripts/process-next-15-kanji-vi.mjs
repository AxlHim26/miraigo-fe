 ...
 * - Ghi lại file, append 15 tên vào data/kanji-processed.txt.
 *
 * Chạy: node scripts/process-next-15-kanji-vi.mjs
...
async function translateBatch(texts, attempt = 1) {
  const prompt = [
    ...
    "Trả về đúng JSON array theo thứ tự, mỗi phần tử là chuỗi tiếng Việt.",
...
async function getVietnameseMeaningForKanji(kanjiChar, attempt = 1) {
  const prompt = `Cho chữ Hán Nhật (kanji): "${kanjiChar}". Trả lời bằng đúng một cụm từ tiếng Việt ngắn là nghĩa phổ biến nhất. Chỉ trả lời cụm từ tiếng Việt.`;
...
// All json in data/kanji
const allFiles = await fs.readdir(dataDir);
const jsonFiles = allFiles.filter((f) => f.endsWith(".json"));
const unprocessed = jsonFiles.filter((f) => !processedSet.has(f));
// Ưu tiên tên ngắn: sort by length of name (without .json) then by name
const sorted = unprocessed
  .map((f) => ({ file: f, nameLen: f.replace(/\.json$/, "").length }))
  .sort((a, b) => a.nameLen - b.nameLen || a.file.localeCompare(b.file));
const toProcess = sorted.slice(0, 15).map((x) => x.file);
...
let append = toProcess.join("\n");
if (processedRaw && !processedRaw.endsWith("\n")) append = "\n" + append;
await fs.appendFile(processedPath, append, "utf-8");
