 ...
 * - Ghi file JSON, append 15 tên vào kanji-processed.txt
 *
 * Chạy: node scripts/process-15-next-kanji-vi.mjs
...
async function translateBatch(texts, attempt = 1) {
  ...
  if (!response.ok) {
    ...
    if (/insufficient balance|hết tiền|số dư/i.test(msg)) {
      console.error("❌ Tài khoản API không đủ số dư.");
      process.exit(1);
    }
    if (attempt < 3) {
      await new Promise((r) => setTimeout(r, 1500 * attempt));
